...
public class BlowfishECB extends Blowfish {
    ...
    /** @see de.org.mchahn.crypto.blowfishj.BlowfishECB#initialize(byte[], int, int) */
    ...
    static final byte[] TESTKEY1 = {
        (byte)0x1c, (byte)0x58, (byte)0x7f, (byte)0x1c,
        (byte)0x13, (byte)0x92, (byte)0x4f, (byte)0xef
    };
    static final long TV_P1 = 0x305532286d6f295aL;
    static final long TV_C1 = 0x55cb3774d13ef201L;

    // test vector #2 (official vector by Bruce Schneier)
    static final String TESTKEY2 = "Who is John Galt?";
    static final long TV_P2 = 0xfedcba9876543210L;
    static final long TV_C2 = 0xcc91732b8022f684L;
    ...
    public static boolean selfTest() {
        ...
        BinConverter.longToByteArray(TV_P1, tvP1b, 0);
        testbf1.encrypt(tvP1b, 0, tvT1b, 0, tvP1b.length);

        if (TV_C1 != BinConverter.byteArrayToLong(tvT1b, 0)) {
            return false;
        }

        testbf1.decrypt(tvT1b, 0, tvT1b, 0, tvT1b.length);

        if (TV_P1 != BinConverter.byteArrayToLong(tvT1b, 0)) {
            return false;
        }

        testbf2 = new BlowfishECB(TESTKEY2.getBytes(), 0, TESTKEY2.length());

        BinConverter.longToByteArray(TV_P2, tvP2b, 0);
        testbf2.encrypt(tvP2b, 0, tvT2b, 0, tvP2b.length);

        if (TV_C2 != BinConverter.byteArrayToLong(tvT2b, 0)) {
            return false;
        }

        testbf2.decrypt(tvT2b, 0, tvT2b, 0, tvT2b.length);

        return TV_P2 == BinConverter.byteArrayToLong(tvT2b, 0);
    }
